...
| events[].description | string | A description for the event trigger. The phrase "when" is added before this description in the ChainFiliates Partner App UI. | false |
...
| unlisted | boolean | Determines whether the Campaign will be displayed on the ChainFiliates Explore Page. Partnership Programs that wish to remain private, and/or include a Whitelist of Partners can opt to set this to `true`. | false |
