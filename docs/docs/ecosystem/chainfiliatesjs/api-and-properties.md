...
# ChainFiliatesJS API & Properties

Instantiating the ChainFiliatesJS library provides a JavaScript object that responds to a few methods. These allow you to manage
referral tokens and execute on conversions.

:::tip
To get more details on object and method types, you can refer to the [**documentation we have generated**](https://ts-docs.js.chainfiliates.so/) for our API and properties.
:::

### `chainfiliates.convert(conversion)`
...
### `chainfiliates.parse(url?, keepQueryParams?)`

This method is used to parse the current URL query parameters to extract and save the ChainFiliates Referral Token. The Query
Parameter `_ushrt` is appended to the Campaign Destination URL automatically when an ChainFiliates Invite Link is visited.&#x20;

:::info
This method is immediately called when ChainFiliatesJS is loaded on a Browser-based Web App.
:::
...
### `chainfiliates.token(campaignReference)`
...
### `chainfiliates.anchor(anchorSelector, campaignReference)`
...
### `chainfiliates.config(config?)`

A method to update the configuration of the ChainFiliatesJS object.
