...
cmd.name("app").description("Admin utils for the Next.js ChainFiliates dApp");

const revalidateCmd = new Command();

revalidateCmd
	.name("revalidate")
	.description("Get Auth Token for use with Next.js ChainFiliates dApp API")
	.option("-k, --key <string>", "DID Key")
	.option("-p, --path <string>", "dApp Path to revalidate")
	.option("-h, --host <string>", "dApp host/origin", "https://app.chainfiliates.so")
