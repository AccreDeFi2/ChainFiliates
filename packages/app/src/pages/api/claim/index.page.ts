...
import { Campaign, RewardTypes } from "ChainFiliatess/campaigns";
...
handler.router.use(withAuth).post(async (req, res) => {
	...
	if (campaignData.chain === Chains.ARWEAVE) {
		...
		const txTags = [
			["Application", "ChainFiliates"],
			["ChainFiliatesCampaign", campaignRef.address],
			["ChainFiliatesPartnerships", partnershipIds.join(", ")]
		];
		...
		try {
			...
			if (campaignData.reward.address) {
				...
				rewardTx.addTag("ChainFiliatesTransferType", "Reward");
				feeTx.addTag("ChainFiliatesTransferType", "Fee");
