...
const chainfiliatesHome = path.join(os.homedir(), ".chainfiliates");
const chainfiliatesConfigFile = path.join(chainfiliatesHome, "config.json");

async function read(): Promise<Storage> {
	try {
		const str = readFileSync(chainfiliatesConfigFile, { encoding: "utf8" });
...
async function write(storage: Storage) {
	if (!existsSync(chainfiliatesHome)) {
		await mkdirSync(chainfiliatesHome);
	}

	writeFileSync(chainfiliatesConfigFile, JSON.stringify(storage, null, 2));
}
